...
— we pick the richest source available.
...
def detect_ecosystem(project_root: Path) -> list[str]:
    ...
    cargo_markers = ["Cargo.toml", "Cargo.lock"]
    maven_markers = ["pom.xml"]

    if any((project_root / m).exists() for m in python_markers):
        ecosystems.append("python")
    if any((project_root / m).exists() for m in npm_markers):
        ecosystems.append("npm")
    if any((project_root / m).exists() for m in cargo_markers):
        ecosystems.append("cargo")
    if any((project_root / m).exists() for m in maven_markers):
...
def _parse_uv_lock_content(
    ...
    if pyproject_content is not None:
        ...
        for dep_str in proj.get("project", {}).get("dependencies", []):
            name = _normalize(re.split(r"[<>=!~\[;]", dep_str)[0].strip())
    ...
    if pyproject_content is not None:
        ...
        for dep_name in proj.get("tool", {}).get("poetry", {}).get("dependencies", {}):
            if dep_name.lower() != "python":
                direct_deps.add(_normalize(dep_name))
        if include_dev:
...
def _parse_uv_lock(lock_path: Path, project_root: Path, include_dev: bool) -> LockfileResult:
    ...
    result = _parse_uv_lock_content(lock_content, pyproject_content, include_dev, str(lock_path))
    return result
...
def _parse_poetry_lock(lock_path: Path, project_root: Path, include_dev: bool) -> LockfileResult:
    ...
    pyproject_content = pyproject.read_text(encoding="utf-8") if pyproject.exists() else None
    result = _parse_poetry_lock_content(lock_content, pyproject_content, include_dev, str(lock_path))
    return result
...
def parse(project_root: Path, ecosystem: str, include_dev: bool = False) -> LockfileResult:
    ...
    return parser(project_root, include_dev)
...
def parse_from_content(
    ...
    """
    content_parsers = {
        "python": _parse_python_from_content,
    }
    parser = content_parsers.get(ecosystem)
    if parser is None:
        return LockfileResult(ecosystem=ecosystem)
    return parser(file_bundle, include_dev)
